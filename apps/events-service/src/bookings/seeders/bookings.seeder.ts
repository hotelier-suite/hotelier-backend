...
export class BookingsSeeder {
  constructor(
    ...
    private bookingRepository: Repository<EventBooking>,
  ...
  async seed() {
    ...
    const bookings = [
      ...
      },
      {
        ...
        venueId:
          venues.find((v) => v.name === 'Grand Ballroom')?.id || venues[0].id,
      },
      {
        ...
        venueId:
          venues.find((v) => v.name === 'Executive Suite')?.id || venues[0].id,
      },
    ];
