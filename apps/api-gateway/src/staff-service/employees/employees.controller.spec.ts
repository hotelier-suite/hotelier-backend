...
describe('EmployeesController (gateway)', () => {
  ...
  it('should getDepartmentStats', async () => {
    mockService.getDepartmentStats.mockReturnValueOnce(of([]));
    const result = await lastValueFrom(controller.getDepartmentStats());
    expect(result).toEqual([]);
  });
