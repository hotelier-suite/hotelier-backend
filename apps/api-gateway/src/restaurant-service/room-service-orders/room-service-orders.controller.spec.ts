...
import { RoomServiceOrdersController } from './';
import { RoomServiceOrdersService } from './room-service-orders.service';

describe('RoomServiceOrdersController (gateway)', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: RoomServiceOrdersService, useValue: mockService }],
    }).compile();
    controller = module.get<RoomServiceOrdersController>(
      RoomServiceOrdersController,
    );
    jest.clearAllMocks();
  });
